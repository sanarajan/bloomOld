...
exports.categories = async (req, res) => {
        ...
        try {
         var catupdate = req.flash('catupdate')
            ...
            res.render('admin/categories', 
                ...
                layout: 'adminLayout',
                pageTitle ,success:req.flash('success'),catupdate});
        } catch (error) {
    ...
    try {
      let  pageTitle = "Add Category"
        res.render("admin/addCategory",{  layout: 'adminLayout',
            pageTitle, userName: req.session.adusername,}); 
  ...
  exports.saveCategory = async (req, res) => {
    try {
     ...
     let  pageTitle = "Add Category"
      ...
      res.render("admin/addCategory", { errorMessage,layout: 'adminLayout',
        pageTitle });
    }
  };
  exports.editCategory = async (req, res) => {
    try {
      ...
      let  pageTitle = "Edit Category"
            ...
            if (category) {
              res.render("admin/editCategory",{ category, userName: req.session.adusername,id, layout: 'adminLayout',
                pageTitle}); 
  ...
  exports.subCategories = async (req, res) => {
    ...
    try {
     ...
     var subcatecss = req.flash('subcatecss')
        ...
        var subcategories = await SubCategory.find({  }).populate('categoryId', 'categoryName').lean();
        res.render('admin/subcategories', 
            {  userName: req.session.adusername,
              subcategories,subcatupdate,
            layout: 'adminLayout',
            pageTitle ,subcatecss});
    } catch (error) {
      console.error("Error in index route:", error);
      res.status(500).send("Internal server error");
    }
      ...
      var categories = await Category.find({  }).lean();
        res.render("admin/addSubCategory",
          {  userName: req.session.adusername,
             layout: 'adminLayout',
            pageTitle,
            categories}); // Renders the login page if no session email is found
  ...
  exports.saveSubCategory = async (req, res) => {
    try {
      ...
      let pageTitle = "Add Sub Category";
      res.render("admin/addSubCategory", { errorMessage, layout: 'adminLayout', pageTitle });
    }
  };
  exports.editSubCategory = async (req, res) => {
    try {
      ...
      let  pageTitle = "Edit Sub Category"
            ...
            const subCategory = await SubCategory.findById(id).populate('categoryId').lean();

            var categorylist = await Category.find({  }).lean();
            if (subCategory) {
              res.render("admin/editSubCategory",{  userName: req.session.adusername,
                subCategory, layout: 'adminLayout',
  ...
  exports.updateSuCategory = async (req, res) => {
    try {           
        const { _id,subCategoryName,categoryId, description, isActive } = req.body;
      ...
      const subCategory = await SubCategory.findOneAndUpdate(
        { _id: subcategoryId },
        { $set: {subCategoryName:subCategoryName,categoryId:categoryId,description:description, isActive: isActive } },
        { new: true } // To return the updated document
      );
  ...
  exports.subcateDelete = async (req, res) => {
    try {
      ...
      if (subCategory) {
        ...
        await subCategory.save();
        return res.status(200).json({ success: true, newStatus: newStatus ? 'Inactive' : 'Active' });
      } else {
