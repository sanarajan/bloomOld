  ...
  try {
    if (!req.session.adusername) {
      ...
      if (user && bcrypt.compareSync(req.body.password, user.password)) {
        ...
        res.render("admin/adminLogin", { // Corrected the path to render adminLogin
...
exports.blockUser = async (req, res) => {
  ...
  if(req.body.task==="block"){
    ...
    return res.status(200).json({ success: true, message: 'User updated successfully',block:pass });
